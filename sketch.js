...
function setup() {
  ...
  invisibleGround = createSprite(width/2,height-145,width,10);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if((keyDown("space")||touches.length>0)&& trex.y >= height-165) {
        trex.velocityY = -20;
        jumpSound.play()
        touches=[]
    }
...
function spawnObstacles(){
 if (frameCount % 80 === 0){
   var obstacle = createSprite(width,height-175,10,40);
...
function spawnClouds() {
  ...
  if (frameCount % 70 === 0) {
     cloud = createSprite(width,85,40,10);
